//! Variant 1 — Night Watch: guide five ships home; one light-seeking creature.
...
pub struct NightWatch {
    ...
    pub creature_spawn: (f32, Vec2, f32),
    pub creature_id: Option<u32>,
    pub target_rescues: usize,
}

impl NightWatch {
    /// Fixed scenario: overlapping arrivals, five rocks shaping the approaches.
    pub fn scenario() -> (Self, Vec<Circle>) {
        let rocks = vec![
            Circle::new(Vec2::new(0.0, 46.0), 5.0),
            Circle::new(Vec2::new(38.0, 22.0), 4.5),
            Circle::new(Vec2::new(-36.0, -26.0), 5.0),
            Circle::new(Vec2::new(22.0, -46.0), 4.5),
            Circle::new(Vec2::new(-52.0, 14.0), 4.5),
        ];
        let schedule = vec![
            Arrival { time: 0.0, name: "Alder", pos: Vec2::new(0.0, 95.0), heading_deg: 180.0 },
            Arrival { time: 22.0, name: "Brant", pos: Vec2::new(95.0, 8.0), heading_deg: 270.0 },
            Arrival { time: 48.0, name: "Cormorant", pos: Vec2::new(-68.0, -66.0), heading_deg: 45.0 },
            Arrival { time: 82.0, name: "Dunlin", pos: Vec2::new(66.0, 68.0), heading_deg: 225.0 },
        ...
        (
            Self {
                schedule,
                next_arrival: 0,
                creature_spawn: (50.0, Vec2::new(-76.0, 28.0), 105.0),
                creature_id: None,
                target_rescues: 3,
            },
            rocks,
...
pub fn step(nw: &mut NightWatch, sea: &mut Sea, dt: f32) {
    ...
    // Arrivals: fixed schedule, capped at three active vessels.
    while let Some(a) = nw.schedule.get(nw.next_arrival) {
        let active = sea.entities.iter().filter(|e| e.is_active_ship()).count();
        if a.time > night_time || active >= t.night_watch_max_active_ships {
            break;
        }
    ...
    if nw.creature_id.is_none() && night_time >= t.night_watch_creature_activation {
        let (_, pos, heading) = nw.creature_spawn;
    ...
    if let Some(cid) = nw.creature_id {
        ...
        if let Some(idx) = sea.entities.iter().position(|e| e.id == cid) {
            let (before, rest) = sea.entities.split_at_mut(idx);
...
pub fn outcome(nw: &NightWatch, sea: &Sea) -> Outcome {
    ...
    if sunk > 0 {
        details.push(format!("{sunk} lost to rocks or the creature."));
    }
